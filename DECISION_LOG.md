...
1-2-1 原始欄位需要支援 `1-2-1`、`121`、`一對一`、`one_to_one`、`one to one`、`O`。Biz Give 亦支援 `biz_give` 等格式。
...
原來的 `vendor_jszip.min.js` 未部署，導致批量下載停在 0。現改用內置 dependency-free ZIP generator，減少漏檔及 CDN 風險。
...
為快速穩定 production，V2 loader 會在核心頁注入模組及 CSS。這是可用的過渡架構；下一次大型 V3 應整合成單一正式 modular app。
