(function(){
  ...
  function drawMemberCard(m,canvas){
    ...
    const actionCount=plan.actions?.length||0;ctx.fillStyle='#211b1e';ctx.font=actionCount>=4?'19px sans-serif':'22px sans-serif';let y=825,lineBudget=actionCount>=4?10:7;
    if(actionCount){for(let i=0;i<Math.min(5,actionCount)&&lineBudget>0;i++){const a=plan.actions[i],maxLines=Math.min(2,lineBudget),r=drawLines(ctx,`${i+1}.【本方案 +${a.gain}分】${a.text}`,92,y,860,actionCount>=4?27:31,maxLines);lineBudget-=r.count;y=r.y+(actionCount>=4?5:9)}}else{const r=drawLines(ctx,'保持現有表現，繼續鞏固本月優勢。',92,y,860,31,2);y=r.y+8}
    let sectionY=y+5;
