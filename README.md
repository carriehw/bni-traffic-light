...
`member.html` — 會員自助入口:輸入分會共用密碼 → 揀自己名 → 睇自己燈號/雷達圖/升燈貼士。
資料經 **AES-256 加密**(`member-data.json`),放上 Pages 都要密碼先解到,唔含生意額。
部署步驟見 [WORKFLOW.md](WORKFLOW.md#5-phase-2--會員自助登入版已建立)。
